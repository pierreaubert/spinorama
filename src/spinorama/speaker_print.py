    ...
    if chart is not None:
        ...
        for ext in ["json", "png"]:  # svg and html skipped to keep size small
            ...
            if ext == "json" and title in (
                "2cols",
                "3cols",
                "SPL Horizontal Contour_smoothed",
                "SPL Vertical Contour_smoothed",
            ...
            if origin == "Princeton" and title in (
                "SPL Horizontal Contour_smoothed",
                "SPL Vertical Contour_smoothed",
            ):
                continue
            ...
            if ext == "png" and (
                (
                    title in ("SPL Horizontal Contour", "SPL Vertical Contour")
                    and origin == "ASR"
                )
                or (
                    title
...
def print_graphs(
    ...
    graphs["SPL Horizontal Contour_smoothed"] = display_contour_smoothed_horizontal(
        df, params
    )
    graphs["SPL Vertical Contour_smoothed"] = display_contour_smoothed_vertical(
        df, params
    )
    ...
    for k in graphs:
        ...
        if graphs[k] is not None:
            graphs[k] = (
                ...
                .configure_title(orient="top", anchor="middle", fontSize=16)
                .properties(
