...
def template_panorama(df, params, speaker, origin, key):
    ...
    if ereflex is not None and hreflex is not None and vreflex is not None:
        ...
        logger.info(
            "Panaroma: ereflex={0} hreflex={1} vreflex={2}".format(
    ...
    if hspl is not None and hcontour is not None and hradar is not None:
        chart &= alt.hconcat(
            hcontour.properties(title="Horizontal SPL"),
            hradar.properties(title="Horizontal SPL"),
            hspl.properties(title="Horizontal SPL"),
        )
    else:
        logger.info(
            "Panaroma: hspl={0} hcontour={1} hradar={2}".format(
    ...
    if vspl is not None and vcontour is not None and vradar is not None:
        chart &= alt.hconcat(
            vcontour.properties(title="Vertical SPL"),
            vradar.properties(title="Vertical SPL"),
            vspl.properties(title="Vertical SPL"),
        )
    else:
        logger.info(
            "Panaroma: vspl={0} vcontour={1} vradar={2}".format(
                vspl is not None, vcontour is not None, vradar is not None
            )
        )
    return chart.configure_legend(orient="top").configure_title(
        orient="top", anchor="middle", fontSize=18
    )
...
def template_sidebyside_eq(df_ref, df_eq, params, speaker, origin, key):
    params2 = scale_params(params, 2)
    logger.debug(
        "params width {0} height {1} xmin {2} xmax {3} ymin {4} ymax {5}".format(
            params2["width"],
            params2["height"],
            params2["xmin"],
            params2["xmax"],
    ...
    for (title, g_ref, g_eq) in [
        ...
        if g_ref is not None:
            if g_eq is not None:
                chart &= alt.hconcat(
                    g_ref.properties(title=speaker), g_eq.properties(title="with EQ")
                ).resolve_scale(color="independent")
            else:
