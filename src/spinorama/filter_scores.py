...
from .compute_cea2034 import compute_cea2034, estimated_inroom_HV,\
    listening_window
...
def scores_print2(score:dict , score1:dict, score2:dict):
    ...
    print('NBD  LW {0:0.2f} {1:0.2f} {2:0.2f}'.format(
        score['nbd_listening_window'], score1['nbd_listening_window'], score2['nbd_listening_window']))
    print('NBD PIR {0:0.2f} {1:0.2f} {2:0.2f}'.format(
        score['nbd_pred_in_room'], score1['nbd_pred_in_room'], score2['nbd_pred_in_room']))
    print('SM  PIR {0:0.2f} {1:0.2f} {2:0.2f}'.format(
        score['sm_pred_in_room'], score1['sm_pred_in_room'], score2['sm_pred_in_room']))
    print('SM   SP {0:0.2f} {1:0.2f} {2:0.2f}'.format(
        score['sm_sound_power'], score1['sm_sound_power'], score2['sm_sound_power']))
    print('LFX       {0:0.0f}   {1:0.0f}   {2:0.0f}'.format(
        score['lfx_hz'], score1['lfx_hz'], score2['lfx_hz']))
    print('LFQ     {0:0.2f} {1:0.2f} {2:0.2f}'.format(
        score['lfq'], score1['lfq'], score2['lfq']))
    print('----------------------')
    print('Score    {0:0.1f}  {1:0.1f}  {2:0.1f}'.format(
