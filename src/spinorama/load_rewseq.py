            ...
            for l in lines:
                ...
                if len_words == 12 and words[0] == "Filter":
                    ...
                    if kind == "HP" or kind == "HPQ":
                        ...
                        logger.debug(
                            "add IIR peq LOWPASS freq {0}Hz srate {1} Q {2} Gain {3}".format(
                                ifreq, srate, rq, rgain
                            )
                        )
                        peq.append((status, iir))
                    ...
                    if kind == "LS" or kind == "LSC":
                        ...
                        logger.debug(
                            "add IIR peq LOWSHELF freq {0}Hz srate {1} Q {2} Gain {3}".format(
                                ifreq, srate, rq, rgain
                            )
                        )
                        peq.append((status, iir))
                        ...
                        logger.debug(
                            "add IIR peq HIGHSHELF freq {0}Hz srate {1} Q {2} Gain {3}".format(
                                ifreq, srate, rq, rgain
                            )
                        )
                        peq.append((status, iir))
