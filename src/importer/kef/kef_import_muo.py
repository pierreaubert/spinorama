...
def process_copy(dirname, speaker_in, speaker_out):
    ...
    dir_vendor.mkdir(mode=0o755, parents=True, exist_ok=True)
    ...
    for i, name in [(1, "On Axis"), (2, "LW"), (3, "ER"), (4, "SP")]:
        ...
        if not file_in.exists():
            ...
            continue
        file_out = Path(f"{dir_vendor}/{name}.txt")
        copy(file_in, file_out)

    # interpolate data and save the result
    data = kef2dict(dir_vendor.as_posix())
    interpolated = interpolate(data, 345)
    data_save(dir_vendor.as_posix(), interpolated)
