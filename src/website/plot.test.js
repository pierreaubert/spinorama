...
import { computeDims, isDisplayVertical, isDisplayCompact, decode64, decode, setGraphOptions } from './plot.js';
...
describe('setGraphOptions', () => {
    ...
    it('should use computeDims to set layout width and height', () => {
        vi.restoreAllMocks();
        const computeDimsSpy = vi.spyOn(plotJs, 'computeDims');
        const options = setGraphOptions(mockInputGraphsData, 1200, 900, {isGraph: true}, 1);
        expect(computeDimsSpy).toHaveBeenCalledWith(1200, 900, false, false, 1);
        const [expectedW, expectedH] = plotJs.computeDims(1200, 900, false, false, 1);
        expect(options.layout.width).toBe(expectedW);
        expect(options.layout.height).toBe(expectedH);
        computeDimsSpy.mockRestore();
         vi.spyOn(console, 'info').mockImplementation(() => {});
    });
...
describe('Plot-Specific Setter Functions', () => {
    ...
    let mockSpeakerGraphs;
    let mockWidth;
    let mockHeight;

    // Helper to create a basic graph structure for speakerGraphs
    const createSpeakerGraphItem = (title = "Speaker Graph", dataCount = 1, hasLegend2 = false) => {
        const graphItem = {
            data: [],
            layout: {
                title: { text: title },
                shapes: [{type: 'rect', visible: true}], // For testing shape removal in 3D
            }
        };
        ...
        if (hasLegend2) {
            graphItem.layout.legend2 = { title: { font: {} }, font: {} };
        }
        return graphItem;
    };
    ...
    beforeEach(() => {
        ...
        mockSpeakerGraphs = [createSpeakerGraphItem("Graph for Speaker A"), createSpeakerGraphItem("Graph for Speaker B")];

        // console.log('plotJs module in beforeEach:', plotJs); // DEBUG LOG - No longer needed
        vi.spyOn(plotJs, 'setGraphOptions').mockImplementation((graphs, w, h, props, num) => {
    ...
    describe('setCEA2034', () => {
        it('should call setGraphOptions and modify data/layout for CEA2034 specifics', () => {
            mockSpeakerGraphs = [createSpeakerGraphItem("CEA2034 for SpkA", 3, true), createSpeakerGraphItem("CEA2034 for SpkB", 2, true)];
            mockSpeakerGraphs[0].data[0].name = "recommended zone";
            mockSpeakerGraphs[0].data[1].x = [1];

            const result = plotJs.setCEA2034("CEA2034", mockSpeakerNames, mockSpeakerGraphs, mockWidth, mockHeight);

            expect(plotJs.setGraphOptions).toHaveBeenCalled();
            const optionsArg = plotJs.setGraphOptions.mock.calls[0][0];
            expect(optionsArg.length).toBe(2);

            expect(optionsArg[0].data[0].legendgroup).toBe('speaker0');
            expect(optionsArg[0].data[0].legendgrouptitle.text).toBe(mockSpeakerNames[0]);
            expect(optionsArg[1].data[0].legendgroup).toBe('speaker1');
            expect(optionsArg[1].data[0].line.dash).toBe('dashdot');

            expect(optionsArg[0].data[0].visible).toBe('legendonly');
            expect(optionsArg[0].data[1].visible).toBe(false);

            expect(result[0].layout.height).toBeDefined();
            expect(result[0].layout.legend2.y).toBeDefined();

            const initialLayoutWithAnnotations = createSpeakerGraphItem("With Annotations", 1, true);
            initialLayoutWithAnnotations.layout.annotations = [{text: "anno1", visible: true}];
            const resultWithAnnotations = plotJs.setCEA2034("CEA2034", mockSpeakerNames, [initialLayoutWithAnnotations, initialLayoutWithAnnotations], mockWidth, mockHeight);
            expect(resultWithAnnotations[0].layout.annotations[0].visible).toBe(false);
        });
    });

    describe('setGraph', () => {
        it('should call setGraphOptions and apply specific data/layout changes', () => {
            mockSpeakerGraphs[0].data.push({ name: 'Band ±3dB', x:[1], y:[1], legendgroup:'', legendgrouptitle:{text:''}, line:{} });

            const result = plotJs.setGraph("On Axis", mockSpeakerNames, mockSpeakerGraphs, mockWidth, mockHeight);
            ...
            expect(optionsArg[0].data.find(d => d.name === 'Band ±3dB').visible).toBe('legendonly');

            const initialHeight = plotJs.setGraphOptions.mock.results[0].value.layout.height;
            if (["On Axis", "Vertical Reflections"].includes("On Axis")) {
                expect(result[0].layout.height).toBe(initialHeight - (3 * 14));
            }
        });
    });

    describe('setRadar', () => {
        it('should call setGraphOptions and adjust layout for radar plots', () => {
            const result = plotJs.setRadar("SPL Horizontal Radar", mockSpeakerNames, mockSpeakerGraphs, mockWidth, mockHeight);
            expect(plotJs.setGraphOptions).toHaveBeenCalled();
            const optionsArg = plotJs.setGraphOptions.mock.calls[0][0];
            expect(optionsArg[0].data[0].legendgroup).toBe('speaker0');

            expect(result[0].layout.height).toBeGreaterThan(0);
            expect(result[0].layout.margin.t).toBeGreaterThan(0);
        });
    });

    describe('setContour', () => {
        it('should process single contour graph using setGraphOptions', () => {
            const singleSpeakerGraph = [createSpeakerGraphItem("Contour SpkA")];
            const result = plotJs.setContour("SPL Horizontal Contour", ["Speaker A"], singleSpeakerGraph, mockWidth, mockHeight);

            expect(plotJs.setGraphOptions).toHaveBeenCalledTimes(1);
            expect(result.length).toBe(1);
            expect(result[0].layout.shapes).toBeNull();
        });
    ...
    describe('setGlobe', () => {
        it('should transform data to barpolar and call setGraphOptions', () => {
            const globeGraphItem = createSpeakerGraphItem("Globe SpkA");
            // Mock data that needs decoding
            globeGraphItem.data[0].x = {bdata: 'AAAAAQAAAAIAAAADAAA=', dtype: 'f4'};
            globeGraphItem.data[0].y = {bdata: 'AAAAQAAAAEAA', dtype: 'f4'};
            globeGraphItem.data[0].z = {bdata: 'AAAAAAAAAEAAgD8=', dtype: 'f4'};

            const result = plotJs.setGlobe("SPL Horizontal Globe", ["Speaker A"], [globeGraphItem], mockWidth, mockHeight);

            expect(plotJs.setGraphOptions).toHaveBeenCalled();
            const optionsArg = plotJs.setGraphOptions.mock.calls[0][0];
            expect(optionsArg[0].data[0].type).toBe('barpolar');
            expect(optionsArg[0].data[0].r).toBeDefined();
            expect(optionsArg[0].data[0].theta).toBeDefined();
            expect(optionsArg[0].data[0].marker.color).toBeDefined();
            expect(optionsArg[0].data[0].marker.showscale).toBe(true);
        });
    });

    describe('setContour3D', () => {
        it('should call setGraphOptions and nullify layout.shapes', () => {
            mockSpeakerGraphs[0].layout.shapes = [{type: 'line'}];
            const result = plotJs.setContour3D("SPL Horizontal Contour 3D", mockSpeakerNames, mockSpeakerGraphs, mockWidth, mockHeight);
...
describe('setGraphOptions', () => {
    ...
    it('should use computeDims to set layout width and height', () => {
        vi.restoreAllMocks(); // Restore console.info if spied by outer beforeEach for this specific spy
        const computeDimsSpy = vi.spyOn(plotJs, 'computeDims');
        const options = setGraphOptions(mockInputGraphsData, 1200, 900, {isGraph: true}, 1);
        expect(computeDimsSpy).toHaveBeenCalledWith(1200, 900, false, false, 1);
        const [expectedW, expectedH] = plotJs.computeDims(1200, 900, false, false, 1);
        ...
        expect(options.layout.height).toBe(expectedH);
        computeDimsSpy.mockRestore();
         vi.spyOn(console, 'info').mockImplementation(() => {}); // Re-apply for subsequent tests in other describes if needed
    });
...
describe('decode64 and decode', () => {
    ...
    // Base64: AQACAP//AAA=
    const testBytesInt16 = new Uint8Array([1, 0, 2, 0, 255, 255, 0, 0]); // 1, 2, -1, 0
