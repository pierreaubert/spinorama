...
def generate_measurement(
    ...
    if need_update(index_name, index_deps):
        ...
        write_if_different(speaker_content, index_name, force=True)
    ...
    for kind in [freq, contour, radar]:
        for graph_name in kind:
            ...
            if need_update(graph_filename, graph_deps):
                ...
                write_if_different(graph_content, graph_filename, force=True)
...
def main():
    ...
    for file_in in glob("{}/*".format(cpaths.CPATH_DATAS_ICONS)):
        if file_in.endswith((".png", ".jpg", ".jpeg", ".webp", ".svg", ".ico")):
            ...
            copy_if_different(file_in, file_out)
    ...
    for item in WEBSITE_JS_FILES:
        try:
            ...
            if item == "meta":
                item_deps.extend([metadata_json_filename, eqdata_json_filename])
                item_deps.extend(metadata_json_chunks.values())
            if not need_update(item_dist, item_deps):
                logger.debug("Skip unchanged JavaScript bundle %s", item_dist)
                continue

            copy_if_different(item_original, item_post_copy)
            ...
            if flag_optim:
                ...
                if os.path.exists(item_post_terser):
                    copy_if_different(item_post_terser, item_dist)
                else:
                    copy_if_different(item_post_import, item_dist)
            else:
                # copy last file
                copy_if_different(item_post_import, item_dist)
    ...
    try:
        for item_name in (
            ...
            item_filename = cpaths.CPATH_DIST + "/" + item_name
            write_if_different(str(item_content), item_filename, force=False)
    except KeyError as key_error:
